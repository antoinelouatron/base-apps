...
#         self.groups = um.ColleGroup.objects.filter(level=level).order_by("nb")
...
#         now = timezone.now()
#         today = now.date()
#         evs = list(am.PeriodicEvent.objects.filter(subj__level=self.level))
#         evs += list(am.BaseEvent.objects.filter(level=self.level, begin__gte=now))
#         evs += list(am.ToDo.objects.filter(date__gte=today, level=self.level))
...
#             groups = um.ColleGroup.objects.filter(level=self.level).annotate(
