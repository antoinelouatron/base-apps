...
#         change_att = uc.ChangeAttendance(self.level)
#         fset = change_att.get_formset()
#         count = um.ColleGroup.objects.filter(level=self.level).count()
...
#         void_group = um.ColleGroup.objects.create(nb=13, void=True, level=self.level)
#         group_nb = 13
#         change_att = uc.ChangeAttendance(level=self.level)
#         group = um.ColleGroup.objects.get(nb=1, level=self.level)
#         group2 = um.ColleGroup.objects.get(nb=8, level=self.level)
#         groups = list(um.ColleGroup.objects.filter(level=self.level).order_by("nb"))
...
#         group = um.ColleGroup.objects.get(nb=1, level=self.level)
#         group2 = um.ColleGroup.objects.get(nb=7, level=self.level)
...
#         new_void = um.ColleGroup.objects.get(nb=6, level=self.level) # with group_nb == 13
...
#         group_nb = 12
#         groups = list(um.ColleGroup.objects.filter(level=self.level).order_by("nb"))
#         self.assertEqual(len(groups), group_nb)
#         data = self.management_data(group_nb)
#         for i in range(group_nb):
#             data["form-{}-nb".format(i)] = 1 + ((group_nb//2 + i) % group_nb)
#             data["form-{}-id".format(i)] = groups[i].pk
...
#         self.assertEqual(ev.subj.level, self.level)
#         ev.save()
#         url.status = 200
#         resp = url.test()
#         self.assertIn("form", resp.context)
